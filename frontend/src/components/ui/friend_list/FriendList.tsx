...
export default function FriendList() {
  ...
  async function fetchFriends() {
    const response = await fetch(
      `https://localhost:8000/api/friends/get/?id=${session?.user.id}`,
      {
        method: "GET",
      }
    );
  ...
  async function approveFriendRequest(friend: Friend) {
    const response = await fetch(
      `https://localhost:8000/api/friends/approve-friend-request/`,
      {
        method: "PUT",
  ...
  async function deleteFriendship(friend: Friend) {
    const response = await fetch(
      "https://localhost:8000/api/friends/delete-friendship/",
      {
        method: "DELETE",
        headers: { "Content-type": "application/json" },
        body: JSON.stringify({
    ...
    if (response.status === 204) fetchFriends();
  }
  ...
  return (
    <>
      ...
      >
        Friends (
        {
          friendships.filter((friend) => friend.friendship_status === "FRIENDS")
            .length
        }
        )
      </button>
            ...
            friendship.friendship_status === "FRIENDS" && (
              ...
              >
                ...
                <Link href={`/account/${friendship.user.id}`}>
                  <div
                    className="position-relative border border-1 border-dark-subtle rounded-circle me-3"
                    style={{
                      width: "30px",
                      height: "30px",
                      overflow: "hidden",
                ...
                <CustomTooltip text="Unfriend" position="top">
                  <PersonDashFill
                    ...
                    size={35}
                    role="button"
                    onClick={() => deleteFriendship(friendship.user)}
                    color="red"
                  />
                </CustomTooltip>
              </div>
      ...
      >
        ...
        {
          friendships.filter(
            (friendship) => friendship.friendship_status === "REQUEST"
          ).length
        }
        )
      </button>
      ...
      <div
        className="collapse border border-bottom-0"
        id="friendRequestsCollapse"
      >
        {friendships.map(
          (friendship) =>
            friendship.friendship_status === "REQUEST" && (
              ...
              >
                ...
                <Link href={`/account/${friendship.user.id}`}>
                  <div
                    className="me-3 position-relative border border-1 border-dark-subtle rounded-circle"
                    style={{
                      width: "30px",
                      height: "30px",
                      overflow: "hidden",
                    ...
                    <>
                      <CustomTooltip text="Approuve Request" position="top">
                        <CheckCircleFill
                          size={35}
                          role="button"
                          onClick={() => approveFriendRequest(friendship.user)}
                          className="me-1"
                          color="green"
                        />
                      </CustomTooltip>
