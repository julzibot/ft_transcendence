...
export default function SearchPlayerInput({
  ...
  const fetchData = async () => {
    const response = await fetch(
      `https://localhost:8000/api/search-user/?query=${inputValue}&id=${session?.user.id}`,
      {
        method: "GET",
      }
    );
              ...
              <>
                <CustomTooltip text="Accept Request" position="top">
                  <CheckCircleFill
                    ...
                    role="button"
                    className="me-1"
                    onClick={() => approveFriendRequest(user)}
                    color="green"
                  />
                </CustomTooltip>
        ...
        return (
          <CustomTooltip text="Send Friend Request" position="bottom">
            <PersonFillAdd
              size={35}
              role="button"
              onClick={() => handleFriendRequest(session?.user.id, user.id)}
              color="green"
            />
          </CustomTooltip>
        );
  ...
  return (
    <>
        ...
        {searchQuery.length > 0 &&
          searchQuery.map((user) => (
            ...
            >
              ...
              <div className="me-2">{renderStatus(user)}</div>
            </div>
          ))}
      </div>
      ...
      >
        <Toast
          onClose={() =>
            setToastParams((params) => ({ ...params, show: false }))
          }
