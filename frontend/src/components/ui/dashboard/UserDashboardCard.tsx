...
const UserDashboardCard: React.FC<UserDashboardCardProps> = ({ user }) => {
  const [DashboardData, setDashboardData] = useState<DashboardItems | null>(
    null
  );
    ...
    if (user.id) {
      const fetchDashboardDetail = async () => {
        const response = await fetch(
          `https://localhost:8000/api/dashboard/${user.id}`,
          {
            method: "GET",
          }
        );
        if (response.ok) {
          ...
          console.log(
            "[fetchDashboardDetail] Response Status: " + response.status
          );
        }
      };
  ...
  useEffect(() => {
    if (user.id) {
      const fetchUserHistory = async () => {
        const response = await fetch(
          `https://localhost:8000/api/game/history/user/${user.id}`,
          {
            method: "GET",
          }
        );
  ...
  const [activityInstance, setActivityInstance] =
    useState<RefObject<Chart> | null>(null);
  const [scoreInstance, setScoreInstance] = useState<RefObject<Chart> | null>(
    null
  );
  ...
  const updateDate = (instance: RefObject<Chart>, date: Date) => {
    const xScale = instance.current?.options.scales?.["x"] as
      | TimeScale
      | undefined;
    if (xScale) {
          ...
          {DashboardData && dataCreated ? (
            <>
              <h5>
                ...
                {(
                  (DashboardData.wins / DashboardData.games_played) *
                  100
                ).toFixed(0)}
                %)
              </h5>
              <h5>
                Losses: {DashboardData.games_played - DashboardData.wins} (
                {(
                  ((DashboardData.games_played - DashboardData.wins) /
                    DashboardData.games_played) *
                  100
                ).toFixed(0)}
                %)
              </h5>
              ...
              <p className="">
                Number of matches played: {DashboardData.games_played}
              </p>
              <div>
                <button
                  type="button"
                  className="btn btn-info mb-2 mt-1"
              ...
              {dataCreated && (
                <>
                  ...
                  {statsToggle ? (
                    ...
                    <>
                      <label className="btn btn-danger m-2" htmlFor="btn-check">
                        With Stats
                      </label>
                    </>
                  )}
                  <div className="d-flex flex-row m-3">
                    {statsToggle ? (
                      <ActivityChart
                        setChartInstance={setActivityInstance}
                      ...
                      <ScoreChart
                        setChartInstance={setScoreInstance}
                        displayedDate={displayedDate}
                        winData={winData}
                        lossData={lossData}
                        maxY={maxY}
                    ...
                    <GameModesChart
                      setChartInstance={setPieInstance}
                      displayedDate={displayedDate}
                      data={gameModesData}
                    />
                  </div>
              ...
              <div
                ...
                id="history-modal"
                tabIndex={-1}
                aria-labelledby="history-modal-label"
                aria-hidden="true"
              >
                <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                  <div className="modal-content">
                    <div className="modal-header">
                      <h1 className="modal-title fs-5" id="history-modal-label">
                        Game History
                      </h1>
                      <button
                        type="button"
                    ...
                    <div className="modal-body">
                      {Array.isArray(userHistory?.data) &&
                        userHistory?.data
                          .toReversed()
                          .map((obj: GameMatch, index: number) => {
                            let cardColor = "";
                            ...
                            return (
                              <div
                                key={index}
                                className={`match_item match_item_link ${cardColor}`}
                              >
                                <div className="match_item_bg"></div>
                                <div className="match_item_game_mode">
                                  {cardColor}
                                </div>
                                        ...
                                        <tr className="match-table">
                                          ...
                                          >
                                            <div className="d-flex flex-row align-items-center justify-content-evenly">
                                              <div
                                                className="flex-column position-relative border border-4 border-dark-subtle rounded-circle"
                                                style={{
                                                  width: "50px",
                                                  height: "50px",
                                                  overflow: "hidden",
                                              ...
                                              <span
                                                className="d-inline-block flex-column flex-grow-1 overflow-hidden ms-2 fs-4 fw-semibold text-truncate"
                                          ...
                                          {player2 ? (
                                            ...
                                            >
                                              <div className="d-flex flex-row align-items-center justify-content-evenly">
                                                ...
                                                <div
                                                  className="ms-2 position-relative border border-4 border-dark-subtle rounded-circle"
                                                  style={{
                                                    width: "50px",
                                                    height: "50px",
                                                    overflow: "hidden",
