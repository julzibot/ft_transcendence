					...
					tournamentData && tournamentData.map((tournament: Tournament, index: number) => {
						return (
							<div
								...
								className={`${tournament.isStarted && !tournament.participants.some((participant) => participant.user.id === session.user.id) ? 'disabled' : ''} ${tournament.isStarted && tournament.participants.some((participant) => participant.user.id === session.user.id) && 'text-light'
									}
